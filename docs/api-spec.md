...
- 최종 추천 점수는 조건별 점수에 서비스 기본 가중치를 적용한 가중 평균으로 산정한다.
  - 매매가: 35
  - 보증금, 월세: 각 30
  - 지역: 25
  - 면적: 15
  - 건축연도: 10
  - 버스, 지하철역, 병원, CCTV, 공원: 각 6
- 가격 조건은 선호 금액 이하이면 100점이며, 선호 금액을 초과하면 선호 금액의 120%까지 100점에서 0점으로 선형 감점한다.
  - 계산식: `max(0, min(100, round(100 * (1 - (실제금액 - 선호금액) / (선호금액 * 0.2)))))`
  - 단, `실제금액 <= 선호금액`이면 100점으로 처리한다.
- 면적 조건은 선호 면적 이상이면 100점이며, 선호 면적보다 작으면 선호 면적의 80%까지 100점에서 0점으로 선형 감점한다.
  - 계산식: `max(0, min(100, round(100 * (1 - (선호면적 - 실제면적) / (선호면적 * 0.2)))))`
  - 단, `실제면적 >= 선호면적`이면 100점으로 처리한다.
- 건축연도 조건은 선호 연도 이상이면 100점이며, 선호 연도보다 오래되면 10년 차이까지 100점에서 0점으로 선형 감점한다.
  - 계산식: `max(0, min(100, round(100 * (1 - (선호연도 - 실제건축연도) / 10))))`
  - 단, `실제건축연도 >= 선호연도`이면 100점으로 처리한다.
...
로그인한 사용자가 선택한 두 주택의 상세 정보, 거래 이력, 주변시설, 맞춤 적합도, 사용자 맞춤 조건, 관심 주택 정보를 기반으로 AI 비교 결과를 생성한다.
